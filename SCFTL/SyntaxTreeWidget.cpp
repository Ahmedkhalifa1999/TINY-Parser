...
int SyntaxTreeWidget::getTreeWidth(SyntaxTree *tree)
{
    ...
    if (tree->isLeaf() && !tree->hasSibling()) return NODE_WIDTH + NODE_HORIZONTAL_SEPARATION;
    ...
    if (tree->hasSibling()) width += getTreeWidth(tree->getSibling());
...
void SyntaxTreeWidget::drawTree(SyntaxTree *tree, int x, int y)
{
    ...
    if(tree->hasSibling())
    {
        ...
        int siblingX = x + totalWidth/2 + siblingWidth/2;
        int siblingY = y;
